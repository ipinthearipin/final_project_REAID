...
budget = st.sidebar.number_input("💰 Anggaran Bulanan (Rp)", min_value=0.0, value=1000000.0, step=50000.0)

hf_token = st.sidebar.text_input("🔐 Hugging Face Token", type="password", help="Masukkan token Hugging Face anda")
...
with st.form("daily_form"):
    ...
    with col2:
        food = st.number_input("Makanan", min_value=0.0, value=0.0, step=500.0)
    with col3:
        transport = st.number_input("Transportasi", min_value=0.0, value=0.0, step=500.0)
    with col4:
        shopping = st.number_input("Belanja", min_value=0.0, value=0.0, step=500.0)
    with col5:
        others = st.number_input("Lainnya", min_value=0.0, value=0.0, step=500.0)
...
if st.session_state.daily_data:
    ...
    if st.button("🔍 Prediksi Pengeluaran Bulanan"):
        if not hf_token:
            st.warning("🔑 Masukkan token Hugging Face Anda terlebih dahulu di sidebar.")
        else:
            try:
